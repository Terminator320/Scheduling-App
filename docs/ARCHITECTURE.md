...
as unavailable, which is the whole point of the P6 stopgap. It is a client-side
...
the appointment's stored address to the client's **composed** `fullAddress` —
a mismatch means a custom address was saved, so the field opens in custom mode.
...
- **Three harness traps that fail as a HANG or a timeout rather than an
  ...
  `clientsRepositoryProvider` and `appointmentsRepositoryProvider`; a bare
...
Run: `flutter test` (2948 passing as of 2026-08-28 — that is the runner's count;
`grep`ing for `test(`/`testWidgets(` gives fewer (2885), since some cases are
generated inside loops; `functions` adds 1535 jest tests across 67 suites in
