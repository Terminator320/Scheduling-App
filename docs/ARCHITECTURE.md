...
```
lib/
├── main.dart                        App entry, root widget (PaulApp); home = OnboardingGate; AppLock overlay + live account-disable + role-revocation listeners in MaterialApp.builder (the sync wiring lives in core/app/app_sync_listeners.dart; the account-lifecycle listeners stay here — their registration order is load-bearing)
