...
admit. `fieldNotes` is deliberately a second field rather than a shared
`notes` — they are written by different people under different rules, and one
field would mean an assignee's write had to be allowed to overwrite the brief
they were given. The photo pipeline, the offline upload queue and the magic-byte
...
appointments/{docId}
  ...
  fieldNotes: string   What the CREW recorded on site, as distinct from `notes`,
                       ...
                       to be allowed to overwrite their own brief
  ...
  crewStatus: string   What an assignee last signalled on the way — '' or one
                       of `crewStatusRawValues` ('onMyWay' | 'runningLate').
  crewStatusAt: timestamp   Written only through `updateCrewStatus`, by the
  crewStatusBy: string      person named in `crewStatusBy`; the trigger pushes
                       it to active admins who are NOT on the job
  createdAt, updatedAt     server timestamps
