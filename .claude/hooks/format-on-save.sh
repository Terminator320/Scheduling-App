...
# Strip UTF-8 BOM from Dart files (editors saving "UTF-8 with BOM" have
# polluted this repo's diffs before; see .claude/rules/code-quality.md).
if [ "$EXTENSION" = "dart" ]; then
  if [ "$(head -c 3 "$FILE_PATH" | od -An -tx1 | tr -d ' \n')" = "efbbbf" ]; then
    tail -c +4 "$FILE_PATH" > "$FILE_PATH.nobom" && mv "$FILE_PATH.nobom" "$FILE_PATH"
  fi
fi
...
# Dart (dart format is standard; detect Flutter/Dart project via pubspec.yaml).
if [ "$FORMATTED" = false ] && command -v dart >/dev/null 2>&1; then
  case "$EXTENSION" in
    dart)
