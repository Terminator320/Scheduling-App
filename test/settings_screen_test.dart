...
void main() {
  ...
  group('the notifications row reflects the OS status', () {
    // The settings body is a lazy ListView and this row sits well below the
    // fold, so it is never built at the default viewport — hence the tall one
    // rather than a scroll, which would depend on the row's exact offset.
    ...
    testWidgets('reads Off when the OS has not been asked', (tester) async {
      await pumpTall(tester, AuthorizationStatus.notDetermined);
    ...
    testWidgets('reads On once notifications are authorized', (tester) async {
      await pumpTall(tester, AuthorizationStatus.authorized);
